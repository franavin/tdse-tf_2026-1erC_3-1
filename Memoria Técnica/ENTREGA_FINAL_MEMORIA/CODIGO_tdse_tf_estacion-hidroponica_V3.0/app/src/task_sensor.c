...
const task_sensor_cfg_t task_sensor_cfg_list[] ={
	...
	{ID_RLY_FB_PUMP, RLY_FB_PUMP_PORT, RLY_FB_PUMP_PIN, RLY_FB_PUMP_CLOSED, DEL_RLY_MAX, EV_SYS_IDLE, EV_SYS_PUMP_FAULT},
	{ID_RLY_FB_FAN,  RLY_FB_FAN_PORT,  RLY_FB_FAN_PIN,  RLY_FB_FAN_CLOSED,  DEL_RLY_MAX, EV_SYS_IDLE, EV_SYS_FAN_FAULT},
    ...
    switch (p_dta->state)
    {
        ...
        case ST_SNSR_FALLING:
            ...
            if (0 == p_dta->tick)
            {
                if (EV_RAW_DOWN == p_dta->event)
                {
                	LOGGER_INFO(">>> SENSOR: ID %lu confirmado como PRESIONADO!", index);
